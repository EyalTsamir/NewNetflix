...
(function () {
  ...
  let settings = (() => {
    try {
      const saved = JSON.parse(localStorage.getItem('tm-subtitle-settings'));
      if (saved) {
        // מיגרציה: wordPopupWidth + wordPopupFontScale → wordPopupScale
        if (saved.wordPopupScale === undefined && saved.wordPopupFontScale !== undefined) {
          saved.wordPopupScale = saved.wordPopupFontScale;
        }
        delete saved.wordPopupWidth;
        delete saved.wordPopupFontScale;
  ...
  function ensurePopup(id, width) {
    ...
    Object.assign(el.style, {
      ...
      border: '1px solid rgba(255,255,255,0.12)', borderTop: '3px solid #e50914',
  ...
  function hideOriginalSubtitles() {
    ...
    style.textContent = `
      ...
      [class*="timedtext"], [class*="subtitle"], [class*="captions"] { opacity: 0 !important; }
      ...
      .tm-word:active { background: rgba(229,9,20,0.25); }
      ...
      .tm-btn { margin-top: 16px; padding: 12px 20px; border-radius: 10px; border: 1px solid #333; border-left: 3px solid #e50914; background: #141414; color: #fff; cursor: pointer; font-size: 18px; font-weight: 600; transition: all 0.15s ease; display: flex; align-items: center; gap: 8px; }
      .tm-btn:hover { background: #222; box-shadow: 0 0 12px rgba(229,9,20,0.25); border-color: #555; border-left-color: #e50914; }
    ...
    if (!panel || !host.contains(panel)) {
      ...
      sliders.forEach(s => {
        ...
        html += `<div style="margin-bottom:14px;">
          ...
          <input type="range" min="${s.min}" max="${s.max}" step="${s.step}" value="${val}"
            ...
            style="width:100%;accent-color:#e50914;direction:ltr;">
        </div>`;
      });
      ...
      html += `<div style="margin-bottom:14px;">
        ...
        <textarea id="tm-show-context" rows="3" style="width:100%;padding:8px;border-radius:8px;border:1px solid #444;background:#1a1a1a;color:#fff;font-size:13px;resize:vertical;direction:ltr;font-family:Arial,sans-serif;"
          placeholder="e.g. Suits — American legal drama. Harvey Specter (senior partner), Mike Ross (associate with eidetic memory)...">${settings.showContext}</textarea>
...
Keep it under 600 characters. Write in English. Be factual and concise — this is reference metadata, not a review.`;
  ...
  function ensureContextGeneratorPopup() {
    ...
    popup.innerHTML = `
      ...
      <input id="tm-ctxgen-input" type="text" placeholder="e.g. Suits, Breaking Bad, The Office..."
  ...
  function positionBox(box, anchorRect) {
    ...
    const overlayEl = document.getElementById('tm-netflix-subtitle-overlay');
